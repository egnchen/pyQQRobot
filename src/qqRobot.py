...
class qqClient():
    ...
    pollHeaders=dict(referer='http://d1.web2.qq.com/proxy.html?v=20151105001&callback=1&id=2',
        User_Agent='Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.86 Safari/537.36')
    ...
    def QRVeri(self,showQR=None):
        ...
        # cookie proxy
        self.ptwebqq=None
        for item in self.HC.cj:
            if item.name=='p_skey':
                self.HC.setCookie(item.name,item.value,'w.qq.com')
            elif item.name=='p_uin':
                self.HC.setCookie(item.name,item.value,'w.qq.com')
            elif item.name=='ptwebqq':
                self.ptwebqq=item.value
        if self.ptwebqq==None:
            raise RuntimeError('ptwebqq not found. Login aborted.')
    ...
    def Login(self):
        ...
        # post for next second step login
